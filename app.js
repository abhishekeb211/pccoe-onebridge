document.addEventListener('DOMContentLoaded', () => {
    ...
    const roadmapData = [
        { part: "Part 1", title: "Architecture Design", phases: "Requirement Crystallization & AI Strategy Selection" },
        { part: "Part 3", title: "User Identity", phases: "Unified Student Profile & RBAC System" },
        { part: "Part 4", title: "AI Subsystem", phases: "Local Agent Initialization & Gemini API Integration" },
        { part: "Part 7", title: "Scholarships Module", phases: "AI Profile Matcher & Application Tracker" },
        { part: "Part 8", title: "Professional Hub", phases: "Career Inventory & Readiness Scorer" },
        { part: "Part 10", title: "Accessibility Layer", phases: "Confidential Subsystems & Disability Protocol" },
    ...
    const fetchData = async () => {
        ...
        try {
            const [curatedRes, sRes, iRes, nRes] = await Promise.all([
                fetch('data/curated_content.json').then(r => r.json()),
                fetch('scraped_data/scholarships.json').then(r => r.json()).catch(() => []),
                fetch('scraped_data/internships.json').then(r => r.json()).catch(() => []),
            ...
            // Artificial delay for Skeleton Showcase (1.2s)
            setTimeout(() => {
                init();
                updateTabGlider();
            }, 1200);
    ...
    const handleSearchFeedback = (counts) => {
        ...
        if (currentCount > 0) {
            ...
            if (otherTabs.length > 0) {
                ...
                const targetLabel = document.querySelector(`[data-tab="${targetTab}"]`).textContent.trim();
                ...
                searchInfoText.textContent = `No matches found anywhere for "${searchQuery}".`;
                searchInfoBar.classList.add('active');
            }
        }
    };
    ...
    const renderHallOfFame = () => {
        ...
        const filteredAchievers = curatedData.achievers.filter(a => {
            const matchesSearch = a.name.toLowerCase().includes(q) || a.achievement.toLowerCase().includes(q);
            const matchesCategory = achieverFilter === 'all' || a.category === achieverFilter;
            return matchesSearch && matchesCategory;
        });

        achieverGrid.innerHTML = filteredAchievers.map(a => `
        ...
        const filteredLegends = curatedData.legends.filter(l => l.name.toLowerCase().includes(q) || l.achievement.toLowerCase().includes(q));
        legendsGrid.innerHTML = filteredLegends.map(l => `
    ...
    const renderScholarships = () => {
        const q = searchQuery.toLowerCase();
        const filteredStatic = curatedData.featured_scholarships.filter(s => s.name.toLowerCase().includes(q) || s.provider.toLowerCase().includes(q));
        staticScholarshipsGrid.innerHTML = filteredStatic.map(s => `
        ...
        const filteredLive = liveScholarships.filter(s => s.name.toLowerCase().includes(q) || s.provider.toLowerCase().includes(q));
        sFeed.innerHTML = filteredLive.slice(0, 8).map(s => renderLiveCard(s, 'scholarship')).join('');

        skillsGrid.innerHTML = curatedData.skill_pathways.map(skill => {
    ...
    const renderHireMe = () => {
        const q = searchQuery.toLowerCase();
        const filteredPlatforms = curatedData.platforms.filter(p => p.name.toLowerCase().includes(q) || p.details.toLowerCase().includes(q));
        platformsGrid.innerHTML = filteredPlatforms.map(p => `
            <div class="resource-card animate">
        ...
        const filteredInterns = liveInternships.filter(i => i.title.toLowerCase().includes(q) || i.company.toLowerCase().includes(q));
        iFeed.innerHTML = filteredInterns.slice(0, 8).map(i => {
    ...
    const renderRoadmap = () => {
        roadmapContainer.innerHTML = roadmapData.map((item, idx) => `
    ...
    const setupFilters = () => {
        ...
        hofFilters.innerHTML = `<div class="chip active" data-filter="all">All Achievers</div>` + 
            categories.map(cat => `<div class="chip" data-filter="${cat}">${cat.charAt(0).toUpperCase() + cat.slice(1)}</div>`).join('');
    ...
    if (supportToggle) supportToggle.addEventListener('click', () => supportModal.classList.add('active'));
    if (closeModal) closeModal.addEventListener('click', () => supportModal.classList.remove('active'));
