...
# 4. GitHub OAuth Endpoints
@app.get('/auth/github/login')
async def github_login(request: StarletteRequest):
    redirect_uri = request.url_for('github_callback')
    return await oauth.github.authorize_redirect(request, redirect_uri)

@app.get('/auth/github/callback')
async def github_callback(request: StarletteRequest):
    token = await oauth.github.authorize_access_token(request)
    user = await oauth.github.parse_id_token(request, token)
    if not user:
        # fallback: fetch user info from /user endpoint
        resp = await oauth.github.get('user', token=token)
        user = resp.json()
    # Store user info in session/cookie (for demo, just redirect with username)
    response = RedirectResponse(url=f"/?user={user.get('login','github')}")
    return response
